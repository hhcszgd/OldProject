...
private let hostName = "https://api.zjlao.com/"//待改回
...
//        AFNetworkReachabilityManager.shared().startMonitoring()
    ...
    static let shareManager : NetworkManager = {
        ...
        mgr.requestSerializer.setValue("5", forHTTPHeaderField: "APPID")
...
//        mgr.requestSerializer.setValue("383B255B-87F7-466C-914A-0B1A35AA5DC3", forHTTPHeaderField: "DID")//先把UID写死
    ...
    var gdNetWorkStatus : AFNetworkReachabilityStatus  {
        mylog(AFNetworkReachabilityManager.shared().networkReachabilityStatus)
...
//                para["keyStr"] = parameters[keyStr]
        ...
        mylog("ddddddddasdfasdf\(para)")
            var url   =  "V2/"
            url = url + urlString
    if urlString == "Login/shop"  || urlString == "Autograph/auth"{//卖家登录 , url特殊处理(不拼rest)
       mylog(urlString)
    }else{
        url = url + "/rest"
    }
...
//        }
    ...
    func initToken(_ success : @escaping (_ result : OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) -> ()) -> () {
        ...
        self.requestJSONDict(RequestType.GET, urlString: url, parameters: para, success: { (result) in
            print(result.data ?? "xxxxxx")
            print(result.msg ?? "xxxxxx")
            print(result.status ?? "xxxxxx")
...
//                GDAlertView.alert("初始化失败 , 请检查网络", image: nil, time: 2, complateBlock: nil)
    ...
    func loginOut(_ success : @escaping (_ result : OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) -> ()) -> () {
        ...
        GDXmppStreamManager.shareXMPP().xmppLoginout()
        UserDefaults.standard.setValue("nil", forKey: "token")//token 赋值为空//不管有没有联网
        let err = NSError(domain: "wrongdomain.com", code: -10001, userInfo: ["reason" : "netWorkError"])
        failure(err)//先返回再说
...
//                UserDefaults.standard.setValue("nil", forKey: "token")//token 赋值为空
    ...
    func login(_ name : String , password : String , success : @escaping (_ result : OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) -> ()) -> () {
        ...
        requestDataFromNewWork(RequestType.POST, urlString: url , parameters: para as [String : AnyObject] , success: { (result) in
            if  let status =  result.status as? Int {
                UserDefaults.standard.setValue(password, forKey: "IMPASSWORD")
                if status > 0 {
                    NotificationCenter.default.post(Notification(name: NSNotification.Name("LoginSuccess")))
    ...
    //卖家登录
    func loginSeller(_ name : String , password : String , success : @escaping (_ result : OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) -> ()) -> () {
        let url = "Login/shop"
        let para = ["username" : name ,  "password" : password ]
        requestDataFromNewWork(RequestType.POST, urlString: url , parameters: para as [String : AnyObject] , success: { (result) in
            if  let status =  result.status as? Int {
...
//        let para = ["username" : name ,  "password" : password ]
     ...
     */
    func gotTencentYunSign(_ success : @escaping (_ result : OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) -> ()) -> () {
        //http://api.zjlao.com/V2/Autograph/auth
        let url = "Autograph/auth"
        let para = [String : AnyObject]()
         requestDataFromNewWork(RequestType.GET, urlString: url, parameters: para, success: { (result) in
            success(result)
        }) { (error) in
    ...
    // MARK: 注释 : 获取历史消息
    func gotHistoryMessage(from : String , to : String , messageID : String , _ success : @escaping (_ result : OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) -> ()) -> () {
        //GET: http://api.zjlao.com/V2/MessageRoaming/rest
        let url = "MessageRoaming"
        if (from.characters.count == 0 || to.characters.count == 0) {
            GDAlertView.alert("参数为空", image: nil, time: 2, complateBlock: {})
        }
        ...
        var myRealIMUsername = to
        var otherIMUsername = from
        if to == Account.shareAccount.name! {
            
            myRealIMUsername = "z~" + to ;
            otherIMUsername = from ;
        }
    
        if from == Account.shareAccount.name! {
            myRealIMUsername = "z~" + from
            otherIMUsername = to ;
        }
        ...
        if messageID.characters.count == 0 {
            para = [
                "username":myRealIMUsername as AnyObject ,
                "bare_peer":otherIMUsername as AnyObject
            ]
        }else{
            para = [
                "username":myRealIMUsername as AnyObject,
                "bare_peer":otherIMUsername as AnyObject,
                "ID":messageID as AnyObject
            ]
        }
        requestDataFromNewWork(RequestType.GET, urlString: url, parameters: para, success: { (result) in
            success(result)
