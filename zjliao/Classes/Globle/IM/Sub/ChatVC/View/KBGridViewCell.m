...
//    }
//    return 24 ;
    NSLog(@"_%@_%d_%ld",[self class] , __LINE__,[self.groupBrows[section] count]);
...
//    _groupBrows = groupBrows;
    ...
    NSLog(@"_%@_%d_%@",[self class] , __LINE__,_groupBrows);
    NSLog(@"_%@_%d_个数%ld",[self class] , __LINE__,_groupBrows.count);
    NSLog(@"_%@_%d_%ld",[self class] , __LINE__,browCountInOnePage);
}

/** UICollectionViewDelegate */
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    NSLog(@"_%@_%d_%@",[self class] , __LINE__,indexPath);
     id model  = self.groupBrows[indexPath.section][indexPath.item];
    if (indexPath.item%23==0 && indexPath.item>0) {
        NSLog(@"_%@_%d_%@",[self class] , __LINE__,@"执行删除");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"CUSTOMDELETE" object:nil];
    }else{
        NSLog(@"_%@_%d_%@",[self class] , __LINE__,@"输入表情");
