...
//    }
    ...
    if (self.backImageURLStr.length>0 ) {
        if ([self.backImageURLStr hasPrefix:@"http"]) {
            ...
            NSURL * url =  [NSURL URLWithString:[NSString stringWithFormat:@"https://i0.zjlao.com/%@",self.backImageURLStr]];

            [self.backImg sd_setImageWithURL:url placeholderImage:nil options:SDWebImageCacheMemoryOnly];
        }
...
-(void)setModel:(OldBaseModel *)model{
    _model = model;
