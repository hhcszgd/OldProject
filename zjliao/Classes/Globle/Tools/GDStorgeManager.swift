...
//    )
    ...
    func gotCurrentDBQueue() -> (FMDatabaseQueue) {
        ...
        let userInfo = Account.shareAccount
        var name = ""
    ...
    func insertHistoryMessage(from: String, to: String, messageID: String , callBack:@escaping ((_ isSucess:Int ,_ insertArr : [GDMessage])->())) -> () {//isSuccess:是否成功 0 是失败 , 非零是成功 ,  resultStr:提示信息
       self.resetCurrentUser()
        NetworkManager.shareManager.gotHistoryMessage(from: from, to: to , messageID: messageID , { (response) in
            mylog(response.data)
            mylog(response.status)
            mylog(response.msg)
            if(response.status == NSNumber.init(value: 200) || response.status == NSNumber.init(value:400) ){//200或400表示成功
...
//                        mylog(resultAny)
            ...
            let  myAccount = Account.shareAccount.name!//
            
            var to_user = to
            if(to_user.hasPrefix("z~") ){
                let range = to_user.range(of: "z~")
                
                to_user =   to_user.replacingCharacters(in: range!, with: "")
            }else if( to_user.hasPrefix("Z~")){
                let range = to_user.range(of: "Z~")
                to_user =  to_user.replacingCharacters(in:range!, with: "")
            }
            var from_user = from

            if(from_user.hasPrefix("z~") ){
                let range = from_user.range(of: "z~")
                from_user =   from_user.replacingCharacters(in: range!, with: "")
            }else if( from_user.hasPrefix("Z~")){
                let range = from_user.range(of: "Z~")
                from_user =  from_user.replacingCharacters(in:range!, with: "")
            }
            ...
            var   otherAccount =  ""
            if(self.currentUser == to_user){
                otherAccount = from_user
            }else{
                otherAccount =   to_user
            }
...
//            mylog(message.elementID)
            ...
            let  fromAccount = from_user;
            let  toAccount =  to_user;
...
//            let isSuccessOption =  (db?.executeStatements(SQLStr))//这种方法插入单引号会失败
        ...
        let gdmessage : GDMessage = GDMessage.init()
        gdmessage.fromAccount = from
        gdmessage.toAccount = to
        mylog(to)
        gdmessage.body = message.body()
        let elementArrTemp = message.elements(forName: "archived")
        var  serverIDForSave = "0";
        for element in elementArrTemp{
            //                DDXMLElement
            let   serverIDOption = element.attributeStringValue(forName: "id")
            if serverIDOption != nil{
        ...
        self.saveRecentContact(message: gdmessage) { (resultCode, resultStr) in
            if(resultCode == 0 ){
                mylog("保存最近联系人失败")
            }else{
                mylog("保存最近联系人成功")
                        ...
                        self.singleGotMessageFromServer(user: user, lastSortKey: lastSortKey, lastServerMsgID: lastServerMsgID, callBack: { (resultcode, gdMessageArr, messageArr ) in
                            if (messageArr.count > 0){
                                ...
                                for (index ,message) in messageArr.enumerated(){
                                    ...
                                    var needUpdateContact = false
                                    if (index == 0 ){
                                        needUpdateContact = true
                                    }
                                    
                                    self.singleInsertFouncation(db: db, message: message, isMax: "min", from: gdmsg.fromAccount, to: gdmsg.toAccount,needUpdateContact : needUpdateContact, callBack: { (resultCode, resultDescrip) in
...
//                    dict["my_account"] = str5!
    ...
    func singleGotMessageFromServer(user:String , lastSortKey : String , lastServerMsgID: String , callBack:@escaping ((_ isSucess:Int ,_ customMessageArr : [GDMessage] , _ xmppMessageArr : [XMPPMessage])->())) {
        ...
        NetworkManager.shareManager.gotHistoryMessage(from: user , to: Account.shareAccount.name! , messageID: lastServerMsgID, { (response) in
            mylog(response.data)
            mylog(response.status)
            mylog(response.msg)
            if(response.status == NSNumber.init(value:200) || response.status == NSNumber.init(value: 400) ){//200或400表示成功
                if let arr = response.data as? Array<Any>{
                    ...
                    for (index ,item ) in arr.enumerated() {
                        var  msg : XMPPMessage = XMPPMessage.init()
                        ...
                        if let msgDict = descItem as? [NSString : AnyObject] {
                            if let toUser = msgDict["to"] as? String{
                                ...
                                if(toUser.contains("@")){
                                    ...
                                    let range  = toUser.range(of: "@")
                                    var user = toUser.substring(to: (range?.lowerBound)!)
                                    toUserStr = user//meiyong
                                    
                                    if(user.hasPrefix("z~") ){
                                        let range = user.range(of: "z~")
                                        
                                        user =   user.replacingCharacters(in: range!, with: "")
                                    }else if( user.hasPrefix("Z~")){
                                        let range = user.range(of: "Z~")
                                        user =  user.replacingCharacters(in:range!, with: "")
                                    }
                                    customMsg.toAccount = user
                                    msg =   XMPPMessage.init(type: "chat", to:  XMPPJID.init(user: user, domain: "jabber.zjlao.com", resource: "iOS"))
                                    mylog(user)
                                }
                            }
                            if let body = msgDict["body"] as? String{
                            ...
                            if let fromUser  = msgDict["from"] as? String{
                                ...
                                if(fromUser.contains("@")){
                                    ...
                                    let range  = fromUser.range(of: "@")
                                    var user = fromUser.substring(to: (range?.lowerBound)!)
                                    fromUserStr = user
                                    if(user.hasPrefix("z~") ){
                                        let range = user.range(of: "z~")
                            ...
                            customMsg.myAccount = Account.shareAccount.name
...
//    func saveRecentContact(db : FMDatabase? ,message : XMPPMessage ,isMax : String, from : String , to : String  , callBack:@escaping ((_ isSucess:Int ,_ resultStr : String)->())) {
    func saveRecentContact(message : GDMessage , callBack:@escaping ((_ isSucess:Int ,_ resultStr : String)->())) {

        self.xmppQueue.inDatabase { (db) in
            let date : NSDate = NSDate.init()
            let timeStamp = date.timeIntervalSince1970
            let currentUser =  Account.shareAccount.name!
            
            mylog(message.toAccount)
            var to_user = message.toAccount!
            if(to_user.hasPrefix("z~")){
                let range = to_user.range(of: "z~")
            ...
            var from_user = message.fromAccount!
            if(from_user.hasPrefix("z~")){
                let range = from_user.range(of: "z~")
            ...
            if(currentUser != to_user){
                other_account_name = to_user
            }else{
                other_account_name = from_user
            }
...
//            }
    ...
    // MARK: 注释 : 插入单条信息
    func singleInsertFouncation( db : FMDatabase? ,message : XMPPMessage ,isMax : String, from : String , to : String  ,needUpdateContact : Bool, callBack:@escaping ((_ isSucess:Int ,_ resultStr : String)->()))  {
        var sortMax : Int = 0;
        var sortMin : Int = 0
        do {
            let resultSet =  try db?.executeQuery("select max(sort_key) as sort from message;", values: nil )
