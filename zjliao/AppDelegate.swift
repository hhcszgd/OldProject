...
//        self.dealWithRemoteNotification(userInfo: userInfo)
        //MARK:设置红点 , 并刷新订单列表页
        KeyVC.share.mainTabbarVC?.tabBar.items?.first?.badgeValue = ""
        NotificationCenter.default.post(name: NSNotification.Name.init("NEWORDERSTATUS"), object: nil , userInfo: nil )
    }
...
//        GDAlertView.alert(userInfo.description, image: nil, time: 4, complateBlock: nil)
    ...
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        JPUSHService.handleRemoteNotification(userInfo)
        mylog("iOS 8 , 9 接收到的远程推送(前后台都走这个方法)\(userInfo)")
    ...
    func dealWithRemoteNotification(userInfo : [AnyHashable : Any]) {
        ...
        if !Account.shareAccount.isLogin {
            ...
            mylog("收到远程推送 , 但未登陆 , 需通过actionkey键设置哪些消息需要登录, 哪些消息不需要登录")
            return
        }
         self.analysisData(apsDict: userInfo)
        /*
        if let aps  = userInfo["aps"] {
            if let apsDict  = aps as? [String : Any] {
                self.analysisData(apsDict: apsDict)
            ...
            }else{mylog("aps  any类型转Dictionary类型失败")}
         
         
        }else{mylog("解析最外层aps失败")}
         */

    }
    
    func analysisData (apsDict : [AnyHashable : Any] ) {
        if let actionkey = apsDict["actionkey"] {
...
//         self.initnetWorkManager()
        ...
        //self.testAlertUUID()//ios10.2,生产模式下同一个开发者账号的uuid相同 , 开发环境下不一样
        return true
    }
    /*
     - (void)testAlertUUID {
...
*/
    func testAlertUUID () {
        let alert = UIAlertController(title: UIDevice.current.identifierForVendor?.uuidString, message: nil , preferredStyle: UIAlertControllerStyle.alert)
        let action = UIAlertAction(title: "quxiao", style: UIAlertActionStyle.cancel) { (action ) in
            alert.dismiss(animated: true , completion: nil );
        }
        alert.addAction(action)
        KeyVC.share.present(alert, animated: true , completion: nil )
    }
...
//        [registrationIDCompletionHandler:]?
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
        /*进入后台后退出IM*/
//        GDXmppStreamManager.shareXMPP().isNeedReconnect = false
//        GDXmppStreamManager.shareXMPP().xmppLoginout()
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
         NetworkManager.shareManager.initToken({ (originalNetDataModel) in}) { (error ) in}
        // Restart any tasks that were paused (or not yet started) while the application was inactive. If the application was previously in the background, optionally refresh the user interface.
        /*重新进入前台继续登录IM*/
//        GDXmppStreamManager.shareXMPP().isNeedReconnect = true
