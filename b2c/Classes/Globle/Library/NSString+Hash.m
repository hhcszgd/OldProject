...
/**处理价格的方法*/
- (NSMutableAttributedString *)dealhomePricefirstFont:(UIFont *)firthFont lastfont: (UIFont *) lastFont{
    NSMutableAttributedString *mutableString;
    NSString *finishString;
    if (![self containsString:@"."]) {
        finishString = dealPrice(self);
        finishString = [NSString stringWithFormat:@"￥%@",finishString];

    }else{
        finishString = [NSString stringWithFormat:@"￥%@",self];
    }
    
    if ([finishString hasSuffix:@".00"]) {
        finishString = [finishString substringWithRange:NSMakeRange(0, finishString.length - 3)];
        
        mutableString = [[NSMutableAttributedString alloc] initWithString:finishString];
        [mutableString addAttributes:@{NSFontAttributeName: firthFont} range:NSMakeRange(0, 1)];
    }else {
        
        if ([finishString hasSuffix:@"0"]) {
            finishString = [finishString substringWithRange:NSMakeRange(0, finishString.length - 1)];
            mutableString = [[NSMutableAttributedString alloc] initWithString:finishString];
            [mutableString addAttributes:@{NSFontAttributeName: firthFont} range:NSMakeRange(0, 1)];
            [mutableString addAttributes:@{NSFontAttributeName: lastFont} range:NSMakeRange(finishString.length - 1, 1)];
        }else {
            //最后一种情况是小数组后面是两个非零数字
            mutableString = [[NSMutableAttributedString alloc] initWithString:finishString];
            [mutableString addAttributes:@{NSFontAttributeName: firthFont} range:NSMakeRange(0, 1)];
            [mutableString addAttributes:@{NSFontAttributeName: lastFont} range:NSMakeRange(finishString.length - 2, 2)];
