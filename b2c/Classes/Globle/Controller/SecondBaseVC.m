...
-(void)setTableView:(UITableView *)tableView{    [super setTableView:tableView];
    [self.view bringSubviewToFront:self.navigationView];
}
-(void)addsubviews
{
    ActionBaseView * navigationView = [[ActionBaseView alloc]initWithFrame:CGRectMake(0, 0, screenW,44+20)];
...
//         [self.naviTitleLabel sizeToFit];
        self.naviTitleLabel.bounds = CGRectMake(0, 0, screenW*0.8, 34);
...
//    CGFloat tempScale = scale<0.6 ? 0.6 : scale;
    self.navigationView.backgroundColor = [[UIColor colorWithHexString:@"ff7800"] colorWithAlphaComponent:scale];
    CGFloat tempScale = scale<0.6 ? 0.6 : scale;
