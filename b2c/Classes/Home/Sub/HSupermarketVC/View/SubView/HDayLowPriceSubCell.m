...
@end
@implementation HDayLowPriceSubCell
...
- (void)setCustomModel:(CustomCollectionModel *)customModel{
    ...
    if (customModel.full_name.length == 0) {
        NSString *title = [NSString stringWithFormat:@"[热销]%@",customModel.short_name];
        NSMutableAttributedString *short_name = [[NSMutableAttributedString alloc] initWithString:title];
        [short_name addAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:11 * SCALE],NSFontAttributeName,[UIColor colorWithHexString:@"ff3a30"],NSForegroundColorAttributeName, nil] range:NSMakeRange(0, 4)];
        [short_name addAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:11 * SCALE],NSFontAttributeName,[UIColor colorWithHexString:@"333333"],NSForegroundColorAttributeName, nil] range:NSMakeRange(4, title.length -4)];
        
        self.title.attributedText = short_name;
    }else{
