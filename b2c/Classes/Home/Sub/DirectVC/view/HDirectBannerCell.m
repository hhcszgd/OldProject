...
@interface HDirectBannerCell()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@end
@implementation HDirectBannerCell
...
- (UIPageControl *)page{
    if (_page == nil) {
        ...
        _page.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"1da2dd"];
...
- (UICollectionView *)col{
    if (_col == nil) {
        UICollectionView *col = [[UICollectionView alloc] initWithFrame:self.contentView.bounds collectionViewLayout:self.flowLayout];
        _col = col;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        //添加定时器+3
        [self addTimer];
        LOG(@"%@,%d,创建%@",[self class], __LINE__,self)
    }
    return self;
}
#pragma mark -- 添加定时器
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if ([self.delegate respondsToSelector:@selector(HDirectBannerCellActionToActiveWithSubModel:)]) {
        [self.delegate HDirectBannerCellActionToActiveWithSubModel:self.dataArray[indexPath.item]];
    }
}
...
- (void)dealloc{
    ...
    LOG(@"%@,%d,销毁%@",[self class], __LINE__,@"销毁")
}
