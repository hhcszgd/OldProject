...
-(void)setImg:(UIImage *)img{
    ...
    self.imgView.bounds = CGRectMake(screenW/2, screenH / 2 , 0, 0);
    ...
    if ( imgSize.width > screenW && imgSize.height > screenH ) {//宽高都在屏幕以外
        //假设宽就是屏幕的宽  //那么高等于
        CGFloat tempW = screenW;
        CGFloat tempH = imgSize.height * tempW/imgSize.width;
        if (tempH > screenH) {
            imgH = screenH;
            imgW = screenH * imgSize.width/imgSize.height;
        }else{//就这样
            imgW = tempW;
            imgH = tempH;
        }
        ...
        imgW = screenW ;
        imgH = imgSize.height * screenW/imgSize.width;
...
//        UIAlertAction * saveToPhotoLib = [UIAlertAction actionWithTitle:@"保存至相册" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
//        }];
//        
//        [alertVC addAction:saveToPhotoLib];
//        [alertVC addAction:actionCancle];
//        [[GDKeyVC share] presentViewController:alertVC animated:YES completion:nil];
