...
- (void)changCollectionBtnStatus{
    if ([UserInfo shareUserInfo].isLogin) {
        if (self.saveModel.shop_id && (self.saveModel.shop_id.length > 0)) {
            //判断店铺是否被收藏
...
- (void)consult{
    ...
    if (self.saveModel) {
        NSDictionary *dict = @{@"model": self.saveModel, @"action": @"goosChat"};
...
- (void)shar{
    if (self.saveModel) {
        NSDictionary *dict = @{@"model": self.saveModel, @"action": @"shopShar"};
        [[NSNotificationCenter defaultCenter] postNotificationName:GOODSSNETVALUE object:nil userInfo:dict];
    }
...
- (void)actionToAllGood{
    ...
    if (self.saveModel) {
        ...
        NSDictionary *dict = @{@"model": allGoodsModel, @"action": @"allgoods"};
        [[NSNotificationCenter defaultCenter] postNotificationName:GOODSSNETVALUE object:nil userInfo:dict];
    }
...
- (void)actionToshangxin{
    if (self.saveModel) {
        NSDictionary *dict = @{@"model": self.saveModel, @"action": @"shangxin"};
        [[NSNotificationCenter defaultCenter] postNotificationName:GOODSSNETVALUE object:nil userInfo:dict];
    }
...
- (void)actionToShopDeatil{
    if (self.saveModel) {
        ...
        NSDictionary *dict = @{@"model": self.saveModel, @"action": @"toShop"};
        [[NSNotificationCenter defaultCenter] postNotificationName:GOODSSNETVALUE object:nil userInfo:dict];
    }
