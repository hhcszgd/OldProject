...
extension UIColor {
    ...
    class func colorWithString(_ colorStr:String) -> UIColor{
...
func dealPrice(priceStr: String, firstFont: UIFont, lastFont: UIFont) -> NSMutableAttributedString {
    var mutableString: NSMutableAttributedString?
    var finishString: String?
    if let price = Float(priceStr) {
        finishString = String.init(format: "￥%.2f", price / 100.0)
        if let finsishPrice = finishString {
            if finsishPrice.hasSuffix(".00") {
                let startIndex = finsishPrice.startIndex
                let endIndex = finsishPrice.characters.index(finsishPrice.endIndex, offsetBy: -3)
                finishString = finsishPrice.substring(with: (startIndex..<endIndex))
                mutableString = NSMutableAttributedString.init(string: finishString!)
                mutableString?.addAttributes([NSFontAttributeName: firstFont as AnyObject], range: NSMakeRange(0, 1))
                
            } else {
                
                if finsishPrice.hasSuffix("0") {
                    
                    let startIndex = finsishPrice.startIndex
                    let endIndex = finsishPrice.characters.index(finsishPrice.endIndex, offsetBy: -1)
                    finishString = finsishPrice.substring(with: (startIndex..<endIndex))
                    mutableString = NSMutableAttributedString.init(string: finishString!)
                    mutableString?.addAttributes([NSFontAttributeName: firstFont as AnyObject], range: NSMakeRange(0, 1))
                    mutableString?.addAttributes([NSFontAttributeName: lastFont as AnyObject], range: NSMakeRange((finishString?.characters.count)! - 1, 1))
                    ...
                    mutableString = NSMutableAttributedString.init(string: finishString!)
                    mutableString?.addAttributes([NSFontAttributeName: firstFont as AnyObject], range: NSMakeRange(0, 1))
                    mutableString?.addAttributes([NSFontAttributeName: lastFont as AnyObject], range: NSMakeRange((finishString?.characters.count)! - 2, 1))
                }
...
class ZkqSelectSpec: UIView, UICollectionViewDelegate, UICollectionViewDataSource, ZkqSpecStoreFooterDelegate, CAAnimationDelegate {
    ...
    func presentSemiView(semiView: UIView, backView: UIView?, target: UIView?, isScreenShot: Bool) {
        targetView = target
        self.isScreenShot = isScreenShot
        if !((target?.subviews.contains(semiView))!) {
            //设置遮盖模板
            ...
            if isScreenShot {
                let image = addOrUpdateParentScreenshotInView(superView: overlay!, target: target!)
                image.layer.add(animationGroupForward(_froward: true), forKey: "push")

            }
            target?.addSubview(overlay!)
            target?.addSubview(semiView)
    ...
    func viewRemoveFormSuperView() {
        self.searchGoodsDetailModelInGoodsDetailArr { (goodsDetailModel) in
            ...
            if let price = goodsDetailModel.price {
                self.priceLabel.attributedText = dealPrice(priceStr: price, firstFont: UIFont.systemFont(ofSize: 10), lastFont: UIFont.systemFont(ofSize: 10))
            }
        ...
        if delegate != nil {
            ...
            if let isShot = isScreenShot {
                if isShot {
                    let screenshotView = targetView?.viewWithTag(kSemiModalScreenshotTag)
                    screenshotView?.layer.add(animationGroupForward(_froward: false), forKey: "hidden")
                }
            }
            let overlay = targetView?.viewWithTag(kSemiModalOverlayTag)
            let semiView = targetView?.viewWithTag(kSemiModalModalViewTag)
    ...
    func configmentBtn(btn: UIButton, title: String, state: UIControlState, font: CGFloat, titleColor: String, backColor: String, action: Selector) {
        btn.backgroundColor = UIColor.colorWithString(backColor)
        ...
        btn.setTitleColor(UIColor.colorWithString(titleColor), for: state)
    }
...
//    var selectIndexPath: [IndexPath] = [IndexPath]()
    ...
    func presentSemi(goods_id: String) {
        ...
        if delegate != nil {
            delegate?.viewWillDisPlay!()
        }
        NetworkManager.shareManager.requestDataFromNewWork(RequestType.GET, urlString: "SelectSpec", parameters: ["goods_id":goods_id as AnyObject], success: { (result) in
...
//
                ...
                if let spec = self.selectSpecModel?.spec {
                    ...
                    self.searchGoodsDetailModelInGoodsDetailArr(complited: { (goodsDetailModel) in
                        self.goodsImage.sd_setImage(with: imgStrConvertToUrl(goodsDetailModel.image!), placeholderImage: placeholderImage)
                        self.carAnimView.sd_setImage(with: imgStrConvertToUrl(goodsDetailModel.image!), placeholderImage: placeholderImage)
                        self.priceLabel.attributedText  = delePrice(price: goodsDetailModel.price! as NSString)
                    })
                    self.col?.reloadData()
                }
    ...
    var goodsTitle: UILabel = UILabel.creatLabel(GDFont.systemFont(ofSize: 13), textColor: UIColor.colorWithHexString("333333"), backColor: UIColor.clear)
    ...
    func setTopContainer() {
        hiddenBtn = UIButton.init(frame: CGRect.init(x: 0, y: 0, width: screenW, height: screenH * 0.25))
        self.addSubview(hiddenBtn!)
        hiddenBtn?.alpha = 1.0
        hiddenBtn?.tag = hiddenBtntag
        hiddenBtn?.addTarget(self, action: #selector(viewRemoveFormSuperView), for: UIControlEvents.touchUpInside)
        //布局中间的视图
        ...
        goodsImage.layer.borderColor = UIColor.colorWithHexString("000000").cgColor
        goodsImage.backgroundColor = UIColor.white
...
@objc protocol ZkqSpecItemDeletate: class {
    func clickOneItem(cell: ZkqSpecItem, indexPath: IndexPath, btn: UIButton)
}
...
class ZkqSpecItem: BaseColCell {
    ...
    override init(frame: CGRect) {
        ...
        let image = UIImage.creatImageWithColor(color: UIColor.colorWithHexString("f4f4f4"))
    ...
    func configTitleColor(isSelect: Bool) {
        if isSelected {
            titleLabel.textColor = UIColor.colorWithHexString("fefefe")
        }else {
...
class ZkqSpecColHeader: BaseColReusableVIew {
    override init(frame: CGRect) {
        super.init(frame: frame)
...
class ZkqSpecColNormalFooter: BaseColReusableVIew {
...
class ZkqSpecStoreFooter: BaseColReusableVIew {
    override init(frame: CGRect) {
        super.init(frame: frame)
        ...
        subtructBtn?.layer.borderColor = UIColor.colorWithHexString("f4f4f4").cgColor
        subtructBtn?.layer.borderWidth = 1
        subtructBtn?.backgroundColor = UIColor.colorWithHexString("f4f4f4")
        ...
        addBtn?.layer.borderColor = UIColor.colorWithHexString("f4f4f4").cgColor
        addBtn?.layer.borderWidth = 1
        addBtn?.backgroundColor = UIColor.colorWithHexString("f4f4f4")
        
        
        
        countLabel = UILabel.creatLabel(UIFont.systemFont(ofSize: GDCalculator.GDAdaptation(13)), textColor: UIColor.colorWithHexString("333333"), backColor: UIColor.white)
...
//模型
class ZkqSelectSpecModel: BaseModel {
    var goods_id: String?
...
class ZkqTypeDetailModel: BaseModel {
    var quality: String?
...
class ZkqGoodsDetailModel: BaseModel{
    var spec1: String?
    var spec2: String?
    var spec3: String?
    var spec4: String?
