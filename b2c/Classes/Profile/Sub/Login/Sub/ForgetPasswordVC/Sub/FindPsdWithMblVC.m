...
//            [GDAlertView alert:responseObject.msg image:nil  time:3 complateBlock:nil ];
        }
    } failure:^(NSError *error) {
        AlertInVC(@"网络错误")
//        [GDAlertView alert:@"网络错误" image:nil  time:3 complateBlock:nil ];
    }];
...
-(void)findPasswordWithMobileUI
{
    ...
    CGFloat gotAuthButtonW = 120 ;
    /** 验证方式输入框 */
    CustomInputView * authcodeInput  = [[CustomInputView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.topLabel.frame)+margin, self.view.bounds.size.width - gotAuthButtonW, 44)];
...
//    gotAuthcodeButton.layer.masksToBounds = YES;
    ...
    gotAuthcodeButton.frame = CGRectMake(CGRectGetMaxX(authcodeInput.frame), CGRectGetMinY(authcodeInput.frame), gotAuthButtonW, authcodeInput.bounds.size.height);
...
//            hub.mode = MBProgressHUDModeText;
            ...
            AlertInVC(responseObject.msg)
//            [GDAlertView alert:responseObject.msg image:nil  time:3 complateBlock:^{  }];
        }
    } failure:^(NSError *error) {
       AlertInVC(@"网络错误")
//        [GDAlertView alert:@"网络错误" image:nil  time:3 complateBlock:^{ }];
    }];
}
...
-(void)daojishi
{
    ...
    self.gotAuthcodeButton.backgroundColor = [UIColor lightGrayColor];
    ...
    if ([self authcodeLawful:self.authcodeInput.currentText]) {
        [[UserInfo shareUserInfo] checkUserNameOrMobileWithUserName:nil mobile:self.mobileOrEmail email:nil mobilecode:self.authcodeInput.currentText success:^(ResponseObject *responseObject) {
            ...
            if ([responseObject.data isEqualToString:@"verify_sucess"]) {//待整改
                ...
                //已被占用 , 请更改
                AlertInVC(@"验证码不正确");
//                [GDAlertView alert:@"验证码不正确" image:nil  time:3 complateBlock:^{ }];
                result(NO);
            }else if ([responseObject.data isEqualToString:@"verify_overtime"]){
//                [GDAlertView alert:@"验证码已过期" image:nil  time:3 complateBlock:^{   }];
                AlertInVC(@"验证码已过期");
                result(NO);
            }else{
                AlertInVC(@"验证码出现未知错误")
//                [GDAlertView alert:@"验证码出现未知错误" image:nil  time:3 complateBlock:^{ }];
                result(NO);
            }
        } failure:^(NSError *error) {
...
//        [GDAlertView alert:@"请输入手机短信中六位数字的验证码" image:nil  time:3 complateBlock:^{   }];
