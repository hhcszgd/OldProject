...
private let hostName = APIDOMAIN
...
//        mgr.session.configuration.timeoutIntervalForRequest = 6 //timeoutIntervalForRequest时间内, 如果没有请求数据发送,则请求超时
    ...
    private  func requestJSONDict(_ method: RequestType, urlString: String,parameters:[String : AnyObject],success: @escaping (_ result:OriginalNetDataModel) -> () , failure : @escaping (_ error : NSError) ->()) {
        ...
        if method == RequestType.GET {
            //实现GET请求
            get(url , parameters: para , progress: { (progress) in
                mylog(progress)
            }, success: { (sessionTask, result) in
            ...
            //func post(_ URLString: String, parameters: Any?, progress uploadProgress: ((Progress) -> Swift.Void)?, success: ((URLSessionDataTask, Any?) -> Swift.Void)?, failure: ((URLSessionDataTask?, Error) -> Swift.Void)? = nil) -> URLSessionDataTask?
            post(url, parameters: para, progress: { (progress) in
                mylog(progress)
            }, success: { (sessionTask, result) in
            ...
            //put(<#T##URLString: String##String#>, parameters: <#T##Any?#>, success: <#T##((URLSessionDataTask, Any?) -> Void)?##((URLSessionDataTask, Any?) -> Void)?##(URLSessionDataTask, Any?) -> Void#>, failure: <#T##((URLSessionDataTask?, Error) -> Void)?##((URLSessionDataTask?, Error) -> Void)?##(URLSessionDataTask?, Error) -> Void#>)
            put(url , parameters: para , success: { (sessionTask, result ) in
    ...
    //(重构终极自定义)基于核心网络框架的核心方法 进行封装  以后所有的网络请求 都走这个方法(初始化方法除外)
    
    private  func requestServerData(_ method: RequestType, urlString: String,parameters:[String : AnyObject],taskProgress : ((Progress) -> Swift.Void)?,success: ((URLSessionDataTask, Any?) -> Swift.Void)? , failure :  ((URLSessionDataTask?, Error) -> Swift.Void)? = nil) {
        //MARK:在失败回调里 ,如果错误码是-11111  , 代表网络错误  , 测试在自定义控制器里就可以执行showErrorView
        ...
        if method == RequestType.GET {
            ...
            get(url , parameters: para , progress: { (progress) in
                ...
                self.printMessage(urlString, paramete: error as AnyObject?)
                if failure != nil {
                    failure!(sessionTask , error as NSError)
                }
            })
