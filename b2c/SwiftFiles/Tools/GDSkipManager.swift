...
class GDSkipManager: NSObject {
    ...
    class func skip(viewController : UIViewController , model : GDBaseModel){
        ...
        case "goodscollect" , "shopcollect" , "focusbrand" , "pay", "ship", "receive", "comment", "over", "balance", "coupons", "coins", "help" , "order" , "my_capital" , "member_club" , "webpage" , "wap_page" : //webViewVC
...
//            targetVC = QRCodeScannerVC()//先注掉
            break
        case "set":
            //            mylog("跳转到设置")
            //            targetVC = SettingVC(vcType: VCType.withBackButton)
            
//            targetVC = SettingVC()//先注掉
            break
            //        case <#pattern#>:
            //            <#code#>
            //        case <#pattern#>:
...
//            let loginNaviVC = GDLoginNavVC(rootViewController: loginVC)
...
//            targetVC = ChooseLanguageVC()//先注掉
            break
        case "HGoodVC":
                        targetVC = HGoodVC()//先注掉
            break
        case "HShopVC":
