    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0  {
            ...
            if (indexPath.row==0) {
                ...
                model.keyparamete = String.init(format: "%@/AppOrder/Notice.html", WAPDOMAIN) as AnyObject?
            }else if (indexPath.row==1){
                
                model.keyparamete = String.init(format: "%@/AppOrder/logisticsList.html", WAPDOMAIN) as AnyObject?
            }else if (indexPath.row==2){
                model.keyparamete = String.init(format: "%@/AppOrder/promotion.html", WAPDOMAIN) as AnyObject?
            }else if (indexPath.row==3){
                model.keyparamete = String.init(format: "%@/AppOrder/activity.html", WAPDOMAIN) as AnyObject?
            }
