...
class GDKeyVC: UINavigationController  ,UITabBarControllerDelegate , LoginDelegate  , LoginNaviVCDelegate {
    ...
    //    overri  de init(rootViewController: UIViewController) {
    //
    //    }
    //
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self , selector: #selector(testNotifi), name: NSNotification.Name(rawValue:"sb"), object: nil )
        // Do any additional setup after loading the view.
    }
    func testNotifi() {
        GDAlertView.alert("testsg", image: nil, time: 5, complateBlock: nil )
    }
    ...
    func tabBarController(_ tabBarController: UITabBarController, shouldSelect viewController: UIViewController) -> Bool{
        ...
        guard  let _ =  viewController as? ShopCarNavigationVC else {  return true}
...
//            let loginVC = GDLoginVC()
  //          loginVC.loginDelegate = self
    //        let loginNaviVC = GDLoginNavVC.init(rootViewController: loginVC)
            let loginNaviVC : LoginNavVC = LoginNavVC.init(loginNavVC:)()
            loginNaviVC.mydelegate = self ;
    ...
    func tabBarController(_ tabBarController: UITabBarController, didSelect viewController: UIViewController){
        if let bool = self.mainTabbarVC?.selectedViewController?.isKind(of: HomeNavigationVC.self) {
        ...
        if let bool = self.mainTabbarVC?.selectedViewController?.isKind(of: ClassifyNavigationVC.self) {
            if bool && self.priviousVC == viewController {
                NotificationCenter.default.post(name: GDClassifyTabBarReclick, object: nil, userInfo: nil)
                mylog("发送分类重复点击通知")
            }
        }
        if let bool = self.mainTabbarVC?.selectedViewController?.isKind(of: LaoNavigationVC.self) {
            if bool && self.priviousVC == viewController {
                NotificationCenter.default.post(name: GDLaoTabBarReclick, object: nil, userInfo: nil)
                mylog("发送lao重复点击通知")
            }
        }
        if let bool = self.mainTabbarVC?.selectedViewController?.isKind(of: ShopCarNavigationVC.self) {
            if bool && self.priviousVC == viewController {
                NotificationCenter.default.post(name: GDShopcarTabBarReclick, object: nil, userInfo: nil)
                mylog("发送购物车重复点击通知")
            }
        }
        if let bool = self.mainTabbarVC?.selectedViewController?.isKind(of: ProfileNavigationVC.self) {
            if bool && self.priviousVC == viewController {
    ...
    func selectChildViewControllerIndex(index : Int) -> () {
       ...
       // UINavigationController * current = ( UINavigationController * ) [[GDKeyVC share] currentSubVC];
        
        //[current popToRootViewControllerAnimated:NO ];
        //[[GDKeyVC share] selectChildViewControllerIndexWithIndex:4];
        let currentVC = mainTabbarVC?.selectedViewController
        if let currentNavi = currentVC as? UINavigationController {
            currentNavi.popToRootViewController(animated: false)
        }else{
          _ = currentVC?.navigationController?.popToRootViewController(animated: false)
        }
        mainTabbarVC?.selectedIndex = selectedIndex
    }
