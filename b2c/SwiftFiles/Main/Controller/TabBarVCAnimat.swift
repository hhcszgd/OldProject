...
class TabBarVCAnimat: NSObject , UIViewControllerAnimatedTransitioning {
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning){
        ...
        if fromViewController.isKind(of: HomeNavigationVC.self) {
            fromX = -GDDevice.width
            toX = GDDevice.width
        }else if (fromViewController.isKind(of:ClassifyNavigationVC.self)){
            if toViewController.isKind(of: HomeNavigationVC.self) {
                fromX = GDDevice.width
                toX = -GDDevice.width
            }else{
                fromX = -GDDevice.width
        ...
        }else if (fromViewController.isKind(of: LaoNavigationVC.self)){
            if toViewController.isKind(of: HomeNavigationVC.self) || toViewController.isKind(of: ClassifyNavigationVC.self) {
                fromX = GDDevice.width
                toX = -GDDevice.width
            }else{
                fromX = -GDDevice.width
        ...
        }else if (fromViewController.isKind(of: ShopCarNavigationVC.self)){
            if toViewController.isKind(of: ProfileNavigationVC.self) {
                fromX = -GDDevice.width
                toX = GDDevice.width
            }else{
                fromX = GDDevice.width
        ...
        }else if (fromViewController.isKind(of: ProfileNavigationVC.self)){
            fromX = GDDevice.width
            toX = -GDDevice.width
        }
