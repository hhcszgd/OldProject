...
class CustomDetailCell: UIControl {
    ...
    init(frame: CGRect, viewModel: ProfileViewModel) {
        ...
        self.model = viewModel.model.value
        self.setUI()
    ...
    init(frame: CGRect, viewModel: SetViewModel) {
        ...
        setUI()
        viewModel.configView.share().subscribe(onNext: { [weak self](model) in
            self?.configmentView(model: model)
        }, onError: nil, onCompleted: nil, onDisposed: nil)
    }
    func configmentView(model: CustomDetailModel) {
        switch (self.model?.actionKey ?? "") {
        case "clean":
            if model.actionKey == "clean" {
                self.model = model
            }
