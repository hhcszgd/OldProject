...
import UIKit
let publicleftTitleColor = UIColor.colorWithRGB(red: 74, green: 74, blue: 74)
let publicrightTitleColor = UIColor.colorWithRGB(red: 166, green: 166, blue: 166)
class SetTopView: UIView {
    ...
    init(frame: CGRect, viewModel: SetViewModel) {
        ...
        let height: CGFloat = frame.size.height / CGFloat(viewModel.topItems.count)
        let width: CGFloat = frame.size.width
        for (index, model) in viewModel.topItems.enumerated() {
