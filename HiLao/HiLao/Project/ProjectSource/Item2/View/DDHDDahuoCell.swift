...
import UIKit
import  SDWebImage
class DDHDDahuoCell: DDTableViewCell {
...
//        self.backgroundColor = UIColor.randomColor()
    ...
    func addsubviews()  {
        ...
        comment.textColor = UIColor.SubTextColor
...
//                if index == images.count - 1{
        imageContainer.frame = CGRect(x: 0, y: comment.frame.maxY , width: self.bounds.width, height: membersH )
...
//                if index == images.count - 1{
        let imageContainerframe = CGRect(x: 0, y: maxY , width: SCREENWIDTH, height: membersH )
